...
const content = {
  en: {
    title: 'Get a Link Plan',
    subtitle: "We'll review your profile and suggest a link mix.",
    doneTitle: 'Request received!',
    doneBody: "We'll review your site and send a link plan within 1 business day.",
    close: 'Close',
    website: 'Website *',
    budget: 'Budget',
    ...
    mainGoal: 'Main goal',
    goalPlaceholder: 'Select a goal (optional)…',
    goalOptions: [
    ...
    notes: 'Notes',
    optional: '(optional)',
    notesPlaceholder: 'Anything we should know about your campaign',
    error: 'Something went wrong. Please try again.',
    submit: 'Get My Link Plan',
    footer: "No spam. No credit card. We'll reply within 1 business day.",
  },
  uk: {
    title: 'Отримати план посилань',
    subtitle: 'Ми проаналізуємо ваш профіль і запропонуємо мікс посилань.',
    doneTitle: 'Заявку отримано!',
    doneBody: 'Ми перевіримо ваш сайт і надішлемо план посилань протягом 1 робочого дня.',
    close: 'Закрити',
    website: 'Сайт *',
    budget: 'Бюджет',
    budgetPlaceholder: 'Оберіть діапазон бюджету…',
    budgetOptions: [
      { value: 'Under $500', label: 'До $500' },
      { value: '$500–1,000', label: '$500 – $1,000' },
      { value: '$1,000–2,500', label: '$1,000 – $2,500' },
      { value: '$2,500–5,000', label: '$2,500 – $5,000' },
      { value: '$5,000+', label: '$5,000+' },
    ],
    mainGoal: 'Основна мета',
    goalPlaceholder: 'Оберіть мету (необовʼязково)…',
    goalOptions: [
      'Наростити реферальні домени',
      'Покращити комерційні сторінки',
      'Конкурувати з сильнішими доменами',
      'Створити змішаний профіль посилань',
      'Не впевнений',
    ],
    email: 'Email *',
    notes: 'Нотатки',
    optional: '(необовʼязково)',
    notesPlaceholder: 'Щось, що нам варто знати про вашу кампанію',
    error: 'Сталася помилка. Спробуйте ще раз.',
    submit: 'Отримати мій план посилань',
    footer: 'Без спаму. Без кредитної картки. Ми відповімо протягом 1 робочого дня.',
  },
};
      ...
      >
        <div className="bg-gray-950 px-5 pt-5 pb-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-gray-400 text-xs mt-0.5">{c.subtitle}</div>
            </div>
        ...
        {done ? (
          <div className="px-5 py-9 flex flex-col items-center gap-3 text-center">
            ...
            <p className="font-bold text-gray-900">{c.doneTitle}</p>
            <p className="text-gray-400 text-sm">{c.doneBody}</p>
            <button onClick={onClose} className="mt-1 text-xs text-gray-400 hover:text-gray-600 underline underline-offset-2">{c.close}</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="px-5 py-5 space-y-3">
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{c.website}</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{c.budget}</label>
              <select value={budget} onChange={(e) => setBudget(e.target.value)} className={`${inputCls} bg-white cursor-pointer`}>
                ...
                {c.budgetOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{c.mainGoal}</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{c.email}</label>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" className={inputCls} />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">{c.notes} <span className="normal-case font-normal text-gray-300">{c.optional}</span></label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={c.notesPlaceholder} rows={2} className={inputCls} />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 bg-[#F97316] hover:bg-[#EA580C] disabled:opacity-60 text-white font-bold py-3 rounded-xl text-sm transition-colors mt-1">
              {loading ? <Loader2 size={14} className="animate-spin" /> : <>{c.submit} <ArrowRight size={13} /></>}
            </button>
            <p className="text-center text-[11px] text-gray-400">{c.footer}</p>
          </form>
        )}
      </div>
    </div>
